...
EOF
    ...
    if [ "$RELEASE_TYPE" = "Update" ]; then
                      JSON_BODY="### IMA Plugin Support\n\n"
                      JSON_BODY="$JSON_BODY$NEW_TAG\n\n"
          JSON_BODY="$JSON_BODY * upgrade to $NEW_TAG\n\n"
          JSON_BODY="$JSON_BODY #### Gradle\n\n"
                      JSON_BODY="$JSON_BODY * implementation 'com.kaltura.playkit:imaplugin"
      ...
      "sections": [
          {
              "facts": [
                  ...
                  {
                      "name": "Gradle line",
                      "value": "implementation 'com.kaltura.playkit:imaplugin:$COMMIT_SHA'"
                  }
              ]
          }
      ],
