...
public class IMAPlugin extends PKPlugin implements AdsProvider, com.google.ads.interactivemedia.v3.api.AdEvent.AdEventListener, AdErrorEvent.AdErrorListener, ExoPlayerWithAdPlayback.OnAdPlayBackListener {
    ...
    private void addListeners(Player player) {
        messageBus.addListener(this, PlayerEvent.ended, event -> {
            ...
            boolean isLastMidRollPlayed = (adInfo == null) || adInfo.getAdPositionType() == AdPositionType.POST_ROLL || !adCuePoints.hasMidRoll() || (adCuePoints.getAdCuePoints().size() >= 2 && adCuePoints.hasPostRoll() && adInfo != null && adInfo.getAdPodTimeOffset() == adCuePoints.getAdCuePoints().get(adCuePoints.getAdCuePoints().size() - 2));
    ...
    private void requestAdsOnUpdateMedia(String adTagUrl) {
        ...
        if (!isAdvertisingConfigured) {
            requestAdsFromIMA(adTagResponse, adTagUrl);
        }
    }
    ...
    public void onAdError(AdErrorEvent adErrorEvent) {
        ...
        if (adConfig != null) {
            messageBus.post(new AdEvent.AdRequestedEvent(!TextUtils.isEmpty(adConfig.getAdTagUrl()) ? adConfig.getAdTagUrl() : adConfig.getAdTagResponse(), isAutoPlay));
        ...
        switch (adEvent.getType()) {
            ...
            case ALL_ADS_COMPLETED:
                if (!isAdvertisingConfigured) {
                    ...
                    if (getIMAEventsListener() != null) {
                        imaEventsListener.allAdsCompleted();
                    }
